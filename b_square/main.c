...
const float WAYPOINT_CLOSE = 24.0;	// inches
...
int main( int argc, char *argv[] )
{
	...
	if( argc != 9 )
		printf("usage: %s x1 y1 x2 y2 x3 y3 x4 y4\n", argv[0]);
	...
	waypoint_x = atof(argv[3]);		// inches
	waypoint_y = atof(argv[4]);		// inches
	...
	do
	{
		...
		if( target_distance <= WAYPOINT_CLOSE)
		{
			...
			switch( waypoint_count ) 
			{
				case 0:
				{
					waypoint_x = atof(argv[1]);
					waypoint_y = atof(argv[2]);
					break;
				}
				case 1:
				{
					waypoint_x = atof(argv[3]);
					waypoint_y = atof(argv[4]);
					break;
				}
				case 2:
				{
					waypoint_x = atof(argv[5]);
					waypoint_y = atof(argv[6]);
					break;
				}
				case 3:
				{
					waypoint_x = atof(argv[7]);
					waypoint_y = atof(argv[8]);
					break;
				}
			}
		}
