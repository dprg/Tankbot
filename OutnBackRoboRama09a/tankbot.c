...
int run_command(H_tankbot, char *, char *, int);

H_tankbot tb_alloc()
{
        H_tankbot tb = 0;
        tb = (Tankbot *)malloc(sizeof(struct Tankbot));
        if(tb == NULL )
        {       
                perror( "tb_alloc:");
                return NULL;
...
int read_with_timeout( int fd_rd, char *input_buffer, int data_to_read_length, float timeout)
{
        ...
        while ( (io_length < data_to_read_length)) 
        {
                ...
                if( retval == 1)
                {
                        ...
                        //
                        rd_length = read(fd_rd, input_buffer+io_length, data_to_read_length-io_length);

			// debug - print received characters
#if 0
			for (int i=io_length; i<rd_length; i++) {
				printf("0x%x ", input_buffer[i]);
			}
			printf("\n");
#endif
			io_length += rd_length;
                        if(input_buffer[io_length-1]=='\n') {
				// hack to check for double \n in input - don't know why that happens
				if (input_buffer[io_length-2] == '\n')
					io_length--;
				break;
			}
                } else if( retval == 0 )
...
int run_command(H_tankbot tb, char *cmd, char *input_buffer, int data_to_read_length)
{
	...
	// printf("Response: %s", input_buffer);
        ...
        if (!strncmp(cmd, "COM", 3)) {
            if (strncmp(input_buffer, "DIR", 3)) {
                printf("Error in COM command, return was %s\n", input_buffer);
                goto reread;
            }
        }
...
#ifdef SIMULATION
        tb->l_pos += tb->l_vel * 100;
        tb->r_pos += tb->r_vel * 100;
...
void tb_set_motor( H_tankbot tb, int side, float power)
{
        ...
        {
#ifdef SIMULATION 
                // power = 1.0, vel= 20 inch per second
                tb->r_vel = (long)((power * RIGHT_DRAG) / PWM_SCALE) ;
#endif
        ...
        {
#ifdef SIMULATION
                tb->l_vel = (long)((power * LEFT_DRAG) / PWM_SCALE); 
#endif
...
#ifdef SIMULATION
    sensorComm->heading = 0;
#endif

    sensorComm->initialHeading = sensorComm->heading;
...
void tb_read_heading(H_tankbot sensorComm)
{
    ...
    if( headingDelta > 180 )
            ...
            headingDelta += 360;
    // printf("headingDelta: %d\n", headingDelta);

#ifdef SIMULATION
    sensorComm->heading = (headingDelta > 0) ? sensorComm->heading + 2 : sensorComm->heading - 2;

    if (sensorComm->heading > 359) {
        sensorComm->heading -= 360;
    } else if (sensorComm->heading < 0) {
        sensorComm->heading += 360;
    }
    ...
    rv = run_command(sensorComm, (char *)"RCP\n\0", input_buffer, 79);
    if (rv > 0) {
            // printf("RCP output: %s", input_buffer);
            sscanf( input_buffer, "DIR%d\n", &(sensorComm->heading));
    }
